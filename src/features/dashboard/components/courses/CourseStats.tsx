...
export function CourseStats({ total, active, completed, averageProgress }: CourseStatsProps) {
    const stats = [
        { label: 'إجمالي الكورسات', value: total, icon: BookOpen, color: 'var(--color-brand)' },
        { label: 'قيد الدراسة', value: active, icon: Clock, color: 'var(--brand-500)' },
        { label: 'مكتملة', value: completed, icon: CheckCircle, color: '#10b981' },
        { label: 'متوسط الإنجاز', value: `${averageProgress}%`, icon: Award, color: '#8b5cf6' },
    ];
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, index) => (
                ...
                >
                    ...
                    {/* Hover Glow */}
                    <div className="absolute inset-0 bg-gradient-to-br from-brand-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-[2rem]" />
                </motion.div>
            ))}
        </div>
    );
