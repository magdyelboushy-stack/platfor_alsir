...
## التحديثات المعمارية والأمنية (2026)

- وضع معلم واحد
  - تم اعتماد وضع "معلم واحد" حيث يعمل المدير كمعلم، وتم نقل جميع صلاحيات وإدارة المحتوى من مسارات المعلم إلى مسارات المدير.
  - خدمات المصادقة تحدد أن Admin = Teacher عملياً، وتم إزالة جدول المعلمين من الاستخدام الفعّال في مسارات الإدارة.
  - يتم ضبط صلاحيات المساعدين عبر مصفوفة صلاحيات دقيقة، مع وصول شامل للمدير.

- أمان الجلسات
  - تخزين الجلسات في قاعدة البيانات عبر معالج جلسات مخصص، مع حقول user_id وip_address وuser_agent وآخر نشاط.
  - ملفات تعريف الارتباط للجلسة مفعّلة بـ HttpOnly وSecure وSameSite=None لدعم البيئات متعددة المنشأ.
  - التحقق من تغيّر عنوان الـ IP بعدات ذكية وتسجيل ملاحظات الأمان في حال تغيّرات متعددة.
  - إعادة توليد معرّف الجلسة كل 30 دقيقة للحماية من تثبيت الجلسات.
  - سياسة جهاز واحد: عند تسجيل الدخول من جهاز جديد، يتم إنهاء الجلسة السابقة تلقائياً وإرجاع 401.

- حماية CSRF
  - توليد رمز CSRF مرتبط بالجلسة والتحقق منه في الطلبات الحساسة (تسجيل، تسجيل دخول، رسائل التواصل، ردود المدير).
  - مسار الحصول على الرمز: GET ‎/api/csrf-token (يشمل session_id للمطابقة).
  - تسجيل حالات الفشل في File logs لأغراض الاستقصاء.

- CORS
  - السماح لأصول محددة فقط في الإنتاج باستخدام ALLOWED_ORIGINS، ورفض wildcard في الإنتاج.
  - دعم أكثر مرونة في التطوير، مع معالجة طلبات OPTIONS مسبقة.
  - السماح بالرؤوس: Content-Type, Authorization, X-CSRF-Token.

- حماية XSS
  - تفعيل رؤوس الأمان القياسية (X-XSS-Protection، X-Frame-Options، X-Content-Type-Options، CSP صارمة).
  - تنظيف المدخلات وترميزها عبر طبقات متعددة: Middleware للتنظيف، وأدوات Sanitizer، مع فحص أنماط خطرة.
  - التحقق من نوع المحتوى وقيم الحجم للطلبات الواردة.

- تحديد المعدل ومكافحة الإساءة
  - محدد معدل متقدم على أساس عنوان IP والمستخدم، مع القدرة على حظر الـ IP تلقائياً عند الإساءة.
  - محدد معدل يعتمد على SQL لمحاولات تسجيل الدخول الفاشلة مع قفل مؤقت للحساب.

- حماية Honeypot
  - حقول مخفية للكشف عن الروبوتات، تُستخدم في مسارات التسجيل وتسجيل الدخول، مع تسجيل نشاط مشبوه وحظر مؤقت.

- حظر IP
  - نظام حظر قائم على ملف JSON دائم، مع انتهاء صلاحية الحظر المؤقت تلقائياً.
  - يتم تطبيق فحص الحظر مبكراً في سلسلة الوسطاء قبل التوجيه.

- سجلات التدقيق
  - تسجيل إجراءات حساسة مثل تسجيل الدخول والتفعيل وتغيّر الجلسات في جدول سجلات التدقيق، مع بيانات الجهاز وعنوان IP.

- نظام التواصل
  - رسائل البريد: إشعار المدير، تأكيد المستلم، والرد على الرسالة بتصميم عربي واضح متّسق مع الهوية البصرية.
  - تكامل Telegram: إشعار تلقائي برسائل التواصل مع أزرار للرد وتحديث الحالة من داخل المجموعة، ودعم الرد عبر البوت ليُرسل إلى البريد ويُحدّث السجل.
  - تخزين رسائل التواصل في contact_messages مع الحقول: الحالة، رد المدير، الوقت، معرفات رسائل التيليجرام، وقناة الرد المعلّقة.

- مسارات API المتعلقة بالتواصل
  - عام: GET ‎/api/contact لعرض تعليمات وPOST ‎/api/contact لإرسال الرسائل.
  - الإدارة: GET ‎/api/admin/contact-messages لاستعراض الرسائل، ثم POST ‎/api/admin/contact-messages/{id}/status لتحديث الحالة، وPOST ‎/api/admin/contact-messages/{id}/reply لإرسال رد بالبريد (يتطلب CSRF).

- ملاحظات البيئة
  - استخدام APP_ENV وAPP_URL وALLOWED_ORIGINS وMAIL_FROM_ADDRESS وTELEGRAM_BOT_TOKEN في الضبط.
  - يُنصح بمزامنة قيم APP_URL وALLOWED_ORIGINS بين البيئة الخلفية والواجهة الأمامية لضمان عمل CORS بشكل صحيح.

---

## ملخص التغييرات العملية

- نقل صلاحيات المعلم إلى المدير ضمن وضع "معلم واحد"، مع تحديث مسارات الإدارة لتشمل إدارة الكورسات والاختبارات والملفات بالكامل.
- تعزيز طبقات الأمان: جلسات آمنة في قاعدة البيانات، CSRF صارم، CORS مُقنّن، XSS مُخفّف، محدد معدل متقدم، وحظر IP.
- نظام التواصل أصبح موحّداً بواجهة بريدية وهوية بصرية عربية، مع تكامل Telegram لتسريع معالجة الرسائل.

---

## مراجع تقنية سريعة

- جلسات آمنة: app/Utils/SecureSession.php، app/Utils/DatabaseSessionHandler.php
- حماية CSRF: app/Utils/CSRF.php، Controllers/Security/CSRFController.php
- CORS ورؤوس الأمان: app/Middleware/CORS.php، app/Middleware/SecurityHeaders.php
- محدد المعدل والحظر: app/Utils/AdvancedRateLimiter.php، app/Middleware/IPBlocker.php
- حماية XSS والمدخلات: app/Middleware/XSSProtection.php، app/Utils/InputSanitizer.php
- التواصل والبريد: app/Controllers/ContactController.php، app/Services/EmailService.php
- تكامل Telegram: app/Controllers/Webhooks/TelegramWebhook.php، app/Services/TelegramService.php
